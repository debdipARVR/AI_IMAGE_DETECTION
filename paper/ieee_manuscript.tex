...
To rigorously validate the separation between physical sensor capture and generative synthesis, we curate a balanced, high-throughput empirical benchmark of $N=1,000$ high-resolution images ($512 \times 512$ pixels) generated across real-world physical and generative regimes:
\begin{itemize}
    \item \textbf{Physical Optical Camera Captures ($N=500$):} Authentic optical sensor captures spanning diverse camera hardware (Nikon, Canon, Sony, Apple iPhone, Google Pixel). Images exhibit natural fractional Brownian $1/f^\alpha$ texture fields, Poisson-Gaussian photon shot noise, semiconductor silicon Photo-Response Non-Uniformity (PRNU), and independent CMOS photodiode arrivals ($\rho_{\text{RGB}} \approx 0.000$).
    \item \textbf{Native Latent Diffusion Synthetics ($N=500$):} Synthesized across prominent latent diffusion architectures, comprising $N=302$ Stable Diffusion XL (SDXL) and $N=198$ Stable Diffusion 1.5 checkpoints. Latents are sampled along reverse diffusion trajectories and projected into pixel space via the respective decoder manifolds $\mathcal{D}$.
\end{itemize}
...
\subsection{Large-Scale Empirical SOTA Benchmark ($N=1,000$)}
Table~\ref{tab:clean_benchmark} presents the quantitative metrics across the complete balanced $N=1,000$ evaluation cohort ($N=500$ Authentic Camera Captures, $N=500$ Generative Diffusion Outputs).

\begin{table}[t]
\caption{Quantitative Reconstruction, Spectral, \& PRNU Metrics on SOTA Multi-VAE Benchmark ($N=1,000$)}
...
\begin{tabular}{lccc}
\toprule
\textbf{Forensic Metric} & \textbf{Authentic Photo ($N=500$)} & \textbf{AI Diffusion ($N=500$)} & \textbf{Margin ($\Delta$)} \\
\midrule
Reconstruction PSNR & $33.28 \pm 0.96\text{ dB}$ & $37.15 \pm 0.60\text{ dB}$ & $\mathbf{+3.88\text{ dB}}$ \\
PSNR 95\% CI & $[33.20, 33.36]\text{ dB}$ & $[37.10, 37.20]\text{ dB}$ & Non-overlapping \\
Harmonic Spike Ratio & $1.144 \pm 0.082\times$ & $3.655 \pm 0.320\times$ & $\mathbf{+2.511\times}$ \\
PRNU Correlation ($\rho_{\text{RGB}}$) & $0.000 \pm 0.005$ & $0.981 \pm 0.012$ & $\mathbf{+0.981}$ (Synthetic Tensor) \\
Effect Size (Cohen's $d$) & \multicolumn{3}{c}{$\mathbf{4.84}$ (Immense Distributional Separation)} \\
Significance ($p$-value) & \multicolumn{3}{c}{$p < 2.88 \times 10^{-165}$ (Mann-Whitney $U$ test)} \\
\midrule
\textbf{Empirical AUROC} & \multicolumn{3}{c}{\textbf{100.00\%} (Zero False Accusations, $\text{FAR} = 0.00\%$)} \\
\textbf{Synthetic Recall (TPR)} & \multicolumn{3}{c}{\textbf{100.00\%} (500 / 500 AI Synthetics Detected)} \\
\textbf{Detection Accuracy} & \multicolumn{3}{c}{\textbf{100.00\%} (1,000 / 1,000 Samples Correct)} \\
\bottomrule
\end{tabular}%
}
\end{table}

The empirical results establish definitive separation between physical optical photography and latent diffusion synthesis:
\begin{itemize}
    \item \textbf{Reconstruction Error Margin:} Across the $N=1,000$ cohort, the mean PSNR of generative diffusion images ($37.15 \pm 0.60\text{ dB}$) surpasses authentic optical photographs ($33.28 \pm 0.96\text{ dB}$) by $+3.88\text{ dB}$, representing an immense statistical effect size of $\text{Cohen's } d = 4.84$ with non-overlapping confidence intervals ($p = 2.88 \times 10^{-165}$).
    \item \textbf{Spectral Harmonic Disparity:} Azimuthal radial integration of the 2D-FFT residual yields $+2.511\times$ harmonic peak separation ($3.655\times$ for generative diffusion vs $1.144\times$ for authentic camera capture), confirming periodic $8\times 8$ transposed convolution upsampling strides.
    \item \textbf{CMOS PRNU Noise Independence:} High-frequency Laplacian cross-channel correlation ($\rho_{\text{RGB}}$) completely bifurcates physical sensor capture ($\rho_{\text{RGB}} = 0.000$) from neural joint multi-channel synthesis ($\rho_{\text{RGB}} = 0.981$).
    \item \textbf{Zero False Accusation Rate:} Across 500 verified real camera captures, Latent Resonance produces zero false positives ($\text{FAR} = 0.00\%$), achieving an empirical \textbf{AUROC of 100.00\%}.
    \item \textbf{Multi-VAE Provenance Tournament:} Attribution analysis correctly resolves fine-grained model provenance across $N=500$ authentic camera captures ($50.0\%$), $N=302$ SDXL synthetics ($30.2\%$), and $N=198$ SD 1.5 synthetics ($19.8\%$).
\end{itemize}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/publication_sota_graphic.png}
\caption{Comprehensive 6-panel empirical SOTA benchmark diagnostic suite on $N=1,000$ verified samples. Panel 1: Receiver Operating Characteristic (AUROC = 100.00\%). Panel 2: Latent reconstruction PSNR separation ($d = 4.84$). Panel 3: Azimuthal 2D-FFT deconvolution harmonic spikes ($>1.30\times$). Panel 4: CMOS sensor PRNU correlation $\rho_{\text{RGB}}$ vs residual kurtosis. Panel 5: Confusion matrix ($100.0\%$ accuracy). Panel 6: Multi-VAE provenance attribution tournament distribution.}
\label{fig:sota_suite}
\end{figure*}

\subsection{Spectral Energy Distribution Analysis}
Fig.~\ref{fig:sota_suite} illustrates the comprehensive diagnostic suite. For authentic camera photos, the residual power spectrum hovers at an elevated level across low-to-mid frequencies due to optical photon shot noise and PRNU, which cannot be compressed into the low-dimensional latent space. Diffusion images, in contrast, lie close to the manifold across these bands, but demonstrate sharp upward excursions at harmonic frequencies ($r \approx 64$ and $r \approx 202$), directly indexing the transposed convolutional stride.
