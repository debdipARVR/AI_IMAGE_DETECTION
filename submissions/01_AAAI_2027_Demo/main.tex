...
A major obstacle in generative AI provenance is the conflict between commercial intellectual property protection and public verification. Commercial providers guard their multi-billion parameter diffusion backbones. We demonstrate through ScribeMark Image that public verification does not require inspecting the generative backbone. Under the proposed \textbf{Autoencoder Transparency Mandate (ATM)}, regulatory bodies (such as the EU AI Office, US NIST, and C2PA) require foundation model creators to register their pre-trained autoencoder decoder weights $\mathcal{D}$ in an open verification escrow. Because the VAE is strictly an $80\text{M}$-parameter compression codec with zero autonomous generative intelligence, disclosing $\mathcal{D}$ protects proprietary commercial IP while granting the global public a deterministic, zero-shot mechanism for forensic auditability.
