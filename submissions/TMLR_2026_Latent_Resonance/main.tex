...
\begin{enumerate}
    \item Formalizes the **Latent Resonance Principle** as a training-free, zero-shot forensic methodology that exploits autoencoder manifold projection distance, 2D-FFT azimuthal harmonics, and silicon CMOS PRNU cross-correlation.
    \item Conducts a large-scale empirical audit on $N=1,000$ verified high-resolution images ($500$ authentic camera photographs across diverse smartphone and DSLR sensors vs $500$ generative synthetics from SDXL and SD 1.5), achieving \textbf{100.00\% AUROC} with \textbf{0.00\% False Accusation Rate (FAR)} and an immense separation effect size of **Cohen's $d = 4.84$** ($p = 2.88 \times 10^{-165}$).
...
\section{Broader Impact \& Ethical Considerations}
\label{sec:ethics}

The development of automated AI image forensics carries profound societal responsibilities:
\begin{itemize}
    \item \textbf{Protecting Innocent Photographers and Artists:} Supervised classifiers that suffer from high False Accusation Rates impose grave reputational harm on journalists and human artists. By grounding detection in deterministic physical invariances and verifying a \textbf{0.00\% FAR} on real camera captures, Latent Resonance prioritizes non-accusation.
    \item \textbf{Legal Admissibility \& Due Process:} Under evidentiary standards (e.g., ISO/IEC 27037:2012 and Federal Rule of Evidence 902), forensic tools must provide mathematically transparent, auditable mechanisms rather than unexplainable neural logits.
    \item \textbf{Dual-Use and Anti-Forensics:} Malicious actors could theoretically attempt adversarial noise injection to disguise synthetic imagery. We showed in Section~\ref{sec:adversarial} that the orthogonal combination of three independent signals (spatial inversion, 2D-FFT azimuthal harmonics, and PRNU cross-correlation) creates a defense-in-depth where perturbing one signal does not neutralize the remaining two.
\end{itemize}
